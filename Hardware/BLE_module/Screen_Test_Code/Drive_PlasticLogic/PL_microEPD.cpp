...
uint8_t PL_microEPD::readTemperature() {
    uint8_t temp;
    digitalWrite(cs, LOW);
    SPI_Master.transfer(EPD_REGREAD | 0x08);
    temp = SPI_Master.transfer(0xFF);
...
void PL_microEPD::writeBuffer(){
    ...
    digitalWrite(cs, LOW);
    SPI_Master.transfer(0x10);
    for (int i=0; i < EPD_BUFFER_SIZE; i++) SPI_Master.transfer(buffer[i]);
    digitalWrite(cs, HIGH);
    waitForBusyInactive(EPD_TMG_SRT);
}
...
void PL_microEPD::writeRegister(char address, char val1, char val2, char val3, char val4) {
    ...
    SPI_Master.transfer(val1);
    if (val2>-1) SPI_Master.transfer(val2);
    if (val3>-1) SPI_Master.transfer(val3);
    if (val4>-1) SPI_Master.transfer(val4);
    digitalWrite(cs, HIGH);
    waitForBusyInactive(EPD_TMG_SR2);
}
...
void BO_BMA250::begin() {
    ...
    SPI_Master.transfer(0x0F);     // Set g
    SPI_Master.transfer(ACC_GSEL);
    digitalWrite(cs2, HIGH);
    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x10);     // Set bandwith
    SPI_Master.transfer(ACC_BW);
    digitalWrite(cs2, HIGH);
}
...
void BO_BMA250::activateTapOnInt1() {
    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x16);     // Enable single tap interrupt
    SPI_Master.transfer(0x20);
    digitalWrite(cs2, HIGH);
    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x19);     // Map single tap interrupt to Int1 pin
    SPI_Master.transfer(0x20);
    digitalWrite(cs2, HIGH);

    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x2B);     // Adjust tab sensitivity (01..1F, the lower the more sensitive)
    SPI_Master.transfer(0x01);
    digitalWrite(cs2, HIGH);

    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x21);     // Set Interrupt mode = temporary or latched (0x0F)
    SPI_Master.transfer(0x0F);
    digitalWrite(cs2, HIGH);
}
...
void BO_BMA250::clearLatchedInt1() {
    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x21);     // Clear latched Int1
    SPI_Master.transfer(0x80);
    digitalWrite(cs2, HIGH);
}
...
void BO_BMA250::readAccel() {
    digitalWrite(cs2, LOW);
    SPI_Master.transfer(0x02|0x80);
    x = SPI_Master.transfer(0xFF);
    x |= SPI_Master.transfer(0xFF) << 8;
    x >>= 6;
    y = SPI_Master.transfer(0xFF);
    y |= SPI_Master.transfer(0xFF) << 8;
    y >>= 6;
    z = SPI_Master.transfer(0xFF);
    z |= SPI_Master.transfer(0xFF) << 8;
    z >>= 6;
    temp = SPI_Master.transfer(0xFF);
    digitalWrite(cs2, HIGH);
}
